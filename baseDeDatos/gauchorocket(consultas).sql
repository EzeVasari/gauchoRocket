...
select rel.fkEmailCliente as clientes, sum(v.precio) as sumaViaje, sum(ts.precio) as sumaServicio, sum(tc.precio) as sumaCabina
from viaje as v
	inner join ubicacion as u on v.codigo = u.fkCodigoViaje
    inner join reserva as r on u.fkCodigoReserva = r.codigo
    inner join cabina as c on u.fkCodigoCabina = c.codigoCabina
...
select e.matricula as matEquipo, v.nombre as nomViaje,
