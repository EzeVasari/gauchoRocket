$(document).ready(function(){
        ...
        var fieldHTML ="<div class='col-sm-7' id='contenedor1'><label class='col-form-label'>trayecto</label><select class='custom-select' name='trayecto[]'><option selected value='0'>Seleccione trayecto</option> ";
        var consulta="<?php $consulta='SELECT  idTrayecto as codigo, nombreTrayecto as nombre FROM  trayecto '; $resultado = mysqli_query($conexion, $consulta); while($recorrer = mysqli_fetch_assoc($resultado)){ echo ' <option value="'. $recorrer['codigo'] .'">'. $recorrer['nombre'] .'</option>';} ?> </select></div>"; 
